...
def page(report):
    ...
    if error:
        ...
        content = f'<div class="summary"><div class="score"><small>READINESS SCORE</small><strong>{score}<em>/100</em></strong><span>{report["deploymentsAnalyzed"]} deployments analyzed</span></div><div class="metrics">{dimension_markup}<div class="severity-counts"><span><b class="dot high"></b>{high} high</span><span><b class="dot medium"></b>{medium} medium</span><span><b class="dot low"></b>{low} low</span></div></div></div><section class="findings"><div class="section-title"><span>Assessment results <small class="hint">search, filter, expand, download</small></span><small id="resultCount">{len(findings)} open</small></div><div class="results-toolbar"><input id="searchInput" type="search" placeholder="Search rule, resource, evidence..." aria-label="Search assessment results"><select id="severityFilter" aria-label="Filter by severity"><option value="ALL">All severities</option><option value="HIGH">High</option><option value="MEDIUM">Medium</option><option value="LOW">Low</option></select><select id="statusFilter" aria-label="Filter by status"><option value="OPEN">Open findings</option><option value="PASSED">Passed checks</option><option value="ALL">All checks</option></select><button class="tool-button" id="downloadJson" type="button">↓ JSON</button><button class="tool-button" id="downloadCsv" type="button">↓ CSV</button></div><div id="resultList" class="result-list"></div><div class="pagination"><button class="page-button" id="prevPage" type="button">← Previous</button><span id="pageInfo"></span><button class="page-button" id="nextPage" type="button">Next →</button></div></section>'
    catalog_markup = "".join(
    ...
    report_json = json.dumps(report, separators=(",", ":")).replace("<", "\\u003c").replace(">", "\\u003e").replace("&", "\\u0026")
    return """<!doctype html>
...
.results-toolbar{display:grid;grid-template-columns:minmax(180px,1fr) 150px 150px auto auto;gap:8px;padding:12px 0 16px;border-bottom:1px solid var(--line)}.results-toolbar input,.results-toolbar select{min-width:0;padding:10px 11px;border:1px solid var(--line);border-radius:3px;background:#f9faf5;color:var(--ink);font:11px monospace;outline:0}.results-toolbar input:focus,.results-toolbar select:focus{border-color:var(--orange)}.tool-button{padding:10px 11px;background:#edf0e9;color:var(--ink);font:10px monospace;border:1px solid var(--line);white-space:nowrap}.tool-button:hover{background:var(--lime)}.result-list{min-height:180px}.result-row{border-top:1px solid var(--line)}.result-row:first-child{border-top:0}.result-summary{display:flex;align-items:center;gap:12px;padding:15px 0;cursor:pointer;list-style:none}.result-summary::-webkit-details-marker{display:none}.result-summary>div{display:flex;flex:1;flex-direction:column;gap:4px}.result-summary strong{font-size:13px}.result-summary small{color:var(--muted);font:10px monospace}.result-status{width:64px;padding:5px 4px;text-align:center;font:9px monospace;border-radius:2px;background:#f9d3c5;color:#a33e21}.result-status.passed{background:#e5ece5;color:#58746a}.result-detail{display:grid;grid-template-columns:1fr 1fr;gap:17px;padding:6px 0 22px 76px}.result-detail label{display:block;margin-bottom:7px;color:var(--orange);font:9px monospace;letter-spacing:.08em;text-transform:uppercase}.result-detail p{margin:0;color:var(--muted);font-size:12px;line-height:1.6}.result-detail code{display:block;margin-top:7px;padding:8px;background:#edf0e9;color:var(--ink);font:10px monospace;overflow:auto}.empty-results{padding:35px 0;color:var(--muted);text-align:center;font:11px monospace}.pagination{display:flex;align-items:center;justify-content:space-between;padding-top:17px;border-top:1px solid var(--line);color:var(--muted);font:10px monospace}.page-button{padding:9px 11px;background:#edf0e9;color:var(--ink);font:10px monospace}.page-button:disabled{cursor:not-allowed;opacity:.4}.download-note{margin:9px 0 0;color:var(--muted);font:9px monospace}@media(max-width:760px){.results-toolbar{grid-template-columns:1fr 1fr}.results-toolbar input{grid-column:1/-1}.result-detail{grid-template-columns:1fr;padding-left:0}} 
</style></head><body><main class="shell"><header class="top"><div class="brand"><i></i>CNRA <span style="color:#71877e;font-size:10px;letter-spacing:0">/ MVP</span></div><button onclick="location.reload()">Run scan ↗</button></header><div class="eyebrow">Cloud native readiness analyzer</div><section class="hero"><div><h1>How ready is your cluster?</h1><p>A read-only scan of your Kubernetes Deployments against a catalog of 27 practical cloud-native rules.</p></div></section><div class="meta">MODE: __MODE__ · NAMESPACE: __NAMESPACE__ · GENERATED: __TIME__</div>__CONTENT__<section class="catalog"><div class="catalog-title"><span>Rule catalog</span><small>__RULE_COUNT__ rules tracked</small></div><p class="catalog-intro">Every rule is transparent: inspect the field we check, why it matters, and the recommended improvement. This catalog includes passing and open checks.</p>__CATALOG__</section><footer>CNRA MVP · Read-only access · <a href="/healthz" style="color:#f06638">healthz</a> · <a href="/api/catalog" style="color:#f06638">catalog API</a></footer></main><script>const REPORT=__REPORT_JSON__;const PAGE_SIZE=10;let pageNumber=1;const escapeHtml=(value)=>String(value??'').replace(/[&<>"']/g,(char)=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[char]));const checks=(REPORT.services||[]).flatMap((service)=>(service.checks||[]).map((check)=>({...check,resource:service.namespace+'/'+service.name})));const searchInput=document.getElementById('searchInput');const severityFilter=document.getElementById('severityFilter');const statusFilter=document.getElementById('statusFilter');const list=document.getElementById('resultList');const resultCount=document.getElementById('resultCount');const pageInfo=document.getElementById('pageInfo');const prevPage=document.getElementById('prevPage');const nextPage=document.getElementById('nextPage');function filtered(){const q=(searchInput?.value||'').toLowerCase().trim();const severity=severityFilter?.value||'ALL';const status=statusFilter?.value||'OPEN';return checks.filter((item)=>{const haystack=[item.id,item.name,item.resource,item.dimension,item.path,item.why,item.evidence,item.recommendation,item.standard].join(' ').toLowerCase();return (!q||haystack.includes(q))&&(severity==='ALL'||item.severity===severity)&&(status==='ALL'||(status==='OPEN'&&!item.passed)||(status==='PASSED'&&item.passed));});}function render(){const items=filtered();const pages=Math.max(1,Math.ceil(items.length/PAGE_SIZE));if(pageNumber>pages)pageNumber=pages;const visible=items.slice((pageNumber-1)*PAGE_SIZE,pageNumber*PAGE_SIZE);resultCount.textContent=items.length+' result'+(items.length===1?'':'s');pageInfo.textContent='Page '+pageNumber+' of '+pages;prevPage.disabled=pageNumber<=1;nextPage.disabled=pageNumber>=pages;if(!visible.length){list.innerHTML='<div class="empty-results">No results match the current search and filters.</div>';return;}list.innerHTML=visible.map((item)=>{const status=item.passed?'PASSED':'OPEN';return '<details class="result-row"><summary class="result-summary"><span class="result-status '+(item.passed?'passed':'')+'">'+status+'</span><div><strong>'+escapeHtml(item.id)+' · '+escapeHtml(item.name)+'</strong><small>'+escapeHtml(item.resource)+' · '+escapeHtml(item.dimension)+' · '+escapeHtml(item.severity)+'</small></div><b class="expand">+</b></summary><div class="result-detail"><div><label>Why it matters</label><p>'+escapeHtml(item.why)+'</p></div><div><label>Evidence</label><p>'+escapeHtml(item.evidence)+'</p><code>'+escapeHtml(item.path)+'</code></div><div><label>Recommended remediation</label><p>'+escapeHtml(item.recommendation)+'</p></div><div><label>Baseline</label><p>'+escapeHtml(item.standard)+'</p></div></div></details>';}).join('');}function download(filename,content,type){const blob=new Blob([content],{type});const link=document.createElement('a');link.href=URL.createObjectURL(blob);link.download=filename;link.click();URL.revokeObjectURL(link.href);}function csv(){const headers=['status','severity','id','name','dimension','resource','path','evidence','why','recommendation','standard'];const rows=checks.map((item)=>[item.passed?'PASSED':'OPEN',item.severity,item.id,item.name,item.dimension,item.resource,item.path,item.evidence,item.why,item.recommendation,item.standard]);return [headers,...rows].map((row)=>row.map((cell)=>'"'+String(cell??'').replaceAll('"','""')+'"').join(',')).join('\\n');}searchInput?.addEventListener('input',()=>{pageNumber=1;render();});severityFilter?.addEventListener('change',()=>{pageNumber=1;render();});statusFilter?.addEventListener('change',()=>{pageNumber=1;render();});prevPage?.addEventListener('click',()=>{pageNumber--;render();window.scrollTo({top:document.querySelector('.findings').offsetTop-20,behavior:'smooth'});});nextPage?.addEventListener('click',()=>{pageNumber++;render();window.scrollTo({top:document.querySelector('.findings').offsetTop-20,behavior:'smooth'});});document.getElementById('downloadJson')?.addEventListener('click',()=>download('cnra-assessment-'+new Date().toISOString().slice(0,10)+'.json',JSON.stringify(REPORT,null,2),'application/json'));document.getElementById('downloadCsv')?.addEventListener('click',()=>download('cnra-findings-'+new Date().toISOString().slice(0,10)+'.csv',csv(),'text/csv'));render();</script></body></html>""".replace("__MODE__", html.escape(report.get("mode", "unknown"))).replace("__NAMESPACE__", html.escape(str(report.get("namespace", "—")))).replace("__TIME__", html.escape(report.get("generatedAt", "—"))).replace("__CONTENT__", content).replace("__CATALOG__", catalog_markup).replace("__RULE_COUNT__", str(len(catalog))).replace("__REPORT_JSON__", report_json)
